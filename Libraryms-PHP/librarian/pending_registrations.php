...
<html>
	...
	<body>
		<?php
			...
			if($rows == 0)
				echo "<h2 align='center'>Không có tại thời điểm này!</h2>";
			else
			{
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<center><legend>Đang chờ đăng ký thành viên</legend></center>";
				...
				echo "<input type='submit' value='Xác nhận' name='l_confirm' />&nbsp;&nbsp;&nbsp;";
				echo "<input type='submit' value='Từ chối' name='l_delete' />";
				echo "</div>";
				echo "</form>";
			}
			
			$header = 'From: <toan@libraryms.com>' . "\r\n";
			...
			if(isset($_POST['l_confirm']))
			{
				...
				if($members > 0)
					echo success("Thêm thành công! ".$members." tài khoản");
				else
					echo error_without_field("Không đăng ký nào được chọn");
			}
			...
			if(isset($_POST['l_delete']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$to = $email;
						$subject = "Thành viên thư viện bị từ chối";
						$message = "Tư cách thành viên của bạn đã bị thư viện từ chối. Vui lòng liên hệ với thủ thư để biết thêm thông tin.";
						mail($to, $subject, $message, $header);
					}
				}
				if($requests > 0)
